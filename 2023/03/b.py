...
for point in points.values():
    ...
    for touch_point in touch_points:
        if touch_point in special_chars.keys():
            special_chars[touch_point].append(point)
            break

for point, value in special_chars.items():
    if len(value) == 2:
        result += prod([int(point.value) for point in value])
