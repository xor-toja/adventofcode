...
result = 0
winning_dict = defaultdict(lambda: 1)
...
for card_num, line in enumerate(text_input.split('\n'), 1):
    ...
    winning_dict[f"Card {card_num}"] = winning_dict.get(f"Card {card_num}", 1)
    repeats =  winning_dict[f"Card {card_num}"]

    if not winning_set:
        continue

    for win in range(card_num + 1, card_num + len(winning_set) + 2):
        winning_dict[f"Card {win}"] = winning_dict.get(f"Card {win}", 1) + repeats
